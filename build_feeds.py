...
def parse_edition(md_text: str, page_url: str) -> Dict[str, Any]:
    ...
    # - If edition_date known: treat as 06:00 LKT (UTC+5:30) that day.
    ...
    # Short summary: first paragraph of text-only content
    text_only = re.sub(r"<[^>]+>", "", html).strip()
    # Split on double newlines or fall back to full text
    parts = re.split(r"\n\s*\n", text_only, maxsplit=1)
    first_para = parts[0].replace("\n", " ").strip()
...
def load_editions(max_items: int = MAX_ITEMS) -> List[Dict[str, Any]]:
    ...
    # appear later; we will let later entries override earlier ones.
